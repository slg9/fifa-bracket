...
const SEO = {
  fr: {
    ...
    challengeGuide: {
      ...
      description: 'Découvre comment jouer au Brakup Challenge, créer ton bracket Coupe du Monde 2026, marquer des points, sauvegarder ton prono et défier tes amis.',
      image: `${PROD_ORIGIN}/og-image.png`,
...
const EXACT_TRANSLATIONS = new Map<string, string>([
  ...
  ['RELÂCHE DANS LE VERT !', 'RELEASE IN THE GREEN!'],
  ['MAINTIENS LA BALLE, TIRE VERS LE BAS', 'HOLD THE BALL, PULL DOWN'],
  ['TIRE LE ROND JAUNE VERS LE BAS, VISE, PUIS RELÂCHE DANS LE VERT', 'PULL THE YELLOW CIRCLE DOWN, AIM, THEN RELEASE IN THE GREEN'],
  ['BUT !', 'GOAL!'],
  ['ARRÊT !', 'SAVED!'],
  ['RATÉ !', 'MISSED!'],
  ['PASSE !', 'THROUGH!'],
  ...
  ["Le match démarre avec l'équipe que tu touches.", 'The match starts with the team you tap.'],
  ['VAINQUEUR', 'WINNER'],
  ...
  ['Résultat officiel enregistré. Ce match ne peut plus être joué.', 'Official result saved. This match can no longer be played.'],
  ['Score officiel en attente', 'Official score pending'],
  ['Prono réussi', 'Prediction hit'],
  ['Prono raté', 'Prediction missed'],
  ['Dommage', 'Too bad'],
  ['Bien joué', 'Well played'],
  ['points gagnés', 'points earned'],
  ['Vainqueur trouvé', 'Winner found'],
  ['Score exact réussi', 'Exact score hit'],
  ['Buteur trouvé', 'Scorer found'],
...
const REGEX_TRANSLATIONS: Array<[RegExp, string]> = [
  ...
  [/^Ton jeu (.+) est sauvegardé\.(.*)$/, 'Your game $1 is saved.$2'],
  [/^Score exact \+(\d+)$/, 'Exact score +$1'],
  [/^Buteur trouvé \+(\d+)(.*)$/, 'Scorer found +$1$2'],
  [/^#(\d+) · (\d+)\/(\d+)$/, '#$1 · $2/$3'],
]
...
const INLINE_REPLACEMENTS: Array<[RegExp, string]> = [
  ...
  [/\bProno réussi\b/g, 'Prediction hit'],
  [/\bProno raté\b/g, 'Prediction missed'],
  [/\bScore exact\b/g, 'Exact score'],
  [/\bButeur trouvé\b/g, 'Scorer found'],
  [/\bVainqueur trouvé\b/g, 'Winner found'],
